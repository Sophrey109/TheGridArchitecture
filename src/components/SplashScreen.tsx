...
export const SplashScreen = ({ onAnimationComplete }: SplashScreenProps) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsAnimating(true);
      // Quick transition after logo animation completes
      setTimeout(() => {
        onAnimationComplete();
      }, 500); // Just 0.5 seconds after logo finishes animating
  ...
  return (
    ...
    >
      <div 
        className={`font-archivo-black text-6xl md:text-8xl text-primary font-black transition-all duration-1000 ease-in-out ${
