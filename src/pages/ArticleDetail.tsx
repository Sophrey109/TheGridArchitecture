...
const ArticleDetail = () => {
  ...
  const cleanupHTML = (html: string): string => {
    ...
    let result = html
      ...
      // Fix spaces in image URLs by encoding them properly
      .replace(/src="([^"]*\/article-images\/[^"]*)\s+([^"]*\.(?:jpg|jpeg|png|gif|webp|svg))"/gi, (match, beforeSpace, afterSpace) => {
        const fullUrl = beforeSpace + '%20' + afterSpace;
        return `src="${fullUrl}"`;
      })
      // Also handle URLs that already have spaces but aren't encoded
      .replace(/src="([^"]*harrys)\s+(apartment[^"]*\.(?:jpg|jpeg|png|gif|webp|svg))"/gi, 'src="$1%20$2"')
